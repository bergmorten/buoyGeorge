                ...
                <g transform="translate(825.000000, 1100.000000)">
                    ...
                    >
                        ...
                        >
                            ...
                            <use
                                :fill="skinColors[avatar.skinColor]"
                        ...
                        <svg
                            :style="cssVars"
                            v-html="clothesTypes[avatar.clotheType]"
                        />
                        <svg
                            v-if="avatar.clotheType === 'GraphicShirt'"
                            :style="cssVars"
                            v-html="graphicShirtTypes[avatar.graphicType]"
                        />
                        <svg v-html="eyeTypes[avatar.eyeType]" />
                        <svg v-html="mouthTypes[avatar.mouthType]" />
                        <svg v-html="eyebrowTypes[avatar.eyebrowType]" />
                        <svg>
                            <g
                                fill="black"
                        ...
                        <svg
                            :style="cssVars"
                            v-html="topTypes[avatar.topType]"
                        />
                        <svg
                            :style="cssVars"
                            v-html="facialHairTypes[avatar.facialHairType]"
                        />
                        <svg
                            v-html="accessoriesTypes[avatar.accessoriesType]"
                        />
                    </g>
                </g>
            </g>
...
const avatar = computed((): AvataaarModel | null => {
...
const cssVars = computed((): Record<string, string> => {
    const config = avatar.value;
    if (!config) return {};
    return {
        '--avataaar-hair-color': hairColors[config.hairColor],
        '--avataaar-facial-hair-color': hairColors[config.facialHairColor],
        '--avataaar-hat-color': hatAndShirtColors[config.topColor],
        '--avataaar-shirt-color': hatAndShirtColors[config.clotheColor],
    };
});
</script>
