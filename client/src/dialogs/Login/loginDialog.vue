<template>
    ...
    >
        <q-card class="q-dialog-plugin logiq-dialog">
...
const showResetPassword = ref<'ShowButton' | 'ShowForm' | 'none'>('none');
...
onMounted(async () => {
    ...
    try {
        working.value = true;
        await getClients();
        const pastConfig = LocalStorage.getItem<ClientExport>('clientConfig');
        if (pastConfig) {
